...
W25QXX_result_t w25qxx_erase(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint32_t len) {
    ...
    uint8_t buf[w25qxx->sector_size];
    ...
    for (uint32_t sector = first_sector; sector <= last_sector; ++sector) {
        ...
        w25qxx_read(w25qxx, sector*w25qxx->sector_size, buf, w25qxx->sector_size);
        uint32_t buf_addr = (address > sector*w25qxx->sector_size)? address - sector*w25qxx->sector_size : 0;
        uint32_t buf_len = (buf_addr+len > 4096-1)? 4096-buf_addr : len;
        memset(&buf[buf_addr], 0xFF, buf_len);
        len -= buf_len;
        ...
        w25qxx_write(w25qxx, sector*w25qxx->sector_size, buf, w25qxx->sector_size);
